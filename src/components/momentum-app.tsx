...
function Overview({ data }: { data: DashboardPayload }) {
  ...
  const gateColor = oosGate.level === "RED" ? "#a92e26" : oosGate.level === "AMBER" ? "#9a6500" : "#167a4b";
  useEffect(() => {
  ...
  return <div className="dynamic-stack">
    <Section title="Next Action" action={<StateBadge state={state.state} />} onActivate={() => setShowActionGuide(true)}><div style={{ border: `2px solid ${gateColor}`, borderRadius: 12, padding: "14px 16px", marginBottom: 16, background: "#fff", color: "#20231f" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 8 }}><div><span style={{ display: "block", fontSize: 11, fontWeight: 700, letterSpacing: ".08em", color: "#6c716a" }}>OOS ACTION GATE</span><strong style={{ display: "block", marginTop: 2, fontSize: 18 }}>投資行動判定</strong></div><strong style={{ border: `1px solid ${gateColor}`, borderRadius: 999, padding: "5px 10px", color: gateColor, fontSize: 12 }}>{oosGate.level}</strong></div><div style={{ fontSize: 16, fontWeight: 700, lineHeight: 1.55 }}>{oosGate.instruction}</div><p style={{ margin: "6px 0 0", color: "#5f655d", lineHeight: 1.5 }}>{oosGate.reason}</p><div style={{ display: "flex", flexWrap: "wrap", gap: "6px 14px", marginTop: 10, fontSize: 12, color: "#6c716a" }}><span>Phase {oosGate.phase}</span><span>観測 {oosGate.monthsObserved}か月</span><span>OOS MaxDD {data.oos.asOf ? pct(data.oos.stats.maxDrawdown) : "—"}</span><span>OOS CAGR（税引前） {data.oos.asOf ? pct(data.oos.stats.cagr) : "—"}</span></div>{oosGate.blocksNewEntries && <p style={{ margin: "10px 0 0", fontWeight: 700, color: gateColor }}>このOOS停止指示を、下段の通常Next Actionより優先します。</p>}</div><div className="next-action"><div><span>次回注文</span><div className="next-action-copy"><strong>{state.nextAction.type.replaceAll("_", " ")}</strong><p>{state.nextAction.reason}</p></div></div><div><span>対象</span><div className="next-action-copy"><strong>{state.nextAction.symbols.join(" / ") || "—"}</strong><p>{state.nextAction.targetWeights.map((weight) => pct(weight, 0)).join(" / ")}</p></div></div><div><span>注文予定時刻</span><div className="next-action-copy"><strong>{usMarketOpenAt(state.nextAction.executionDate)}</strong></div></div><div><span>N-PORT取込期限</span><div className="next-action-copy"><strong>{data.nportOperations?.nextImportDeadlineAt ? updatedAt(data.nportOperations.nextImportDeadlineAt).slice(0, 16) : "—"}</strong>{data.nportOperations?.universeMode === "FALLBACK" && <p>旧Universeで継続</p>}</div></div></div></Section>
