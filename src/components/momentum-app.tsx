...
import type { DashboardPayload, EquityPoint, PortfolioTarget } from "@/lib/types";
...
function Backtest({data}:{data:DashboardPayload}){const curve=data.backtest.equityCurve;const chart=useMemo(()=>curve.map((x:EquityPoint)=>({date:x.date,equity:x.equity})),[curve]);return <div className="dynamic-stack"><div className="dynamic-metric-grid"><Metric label="Historical CAGR" value={pct(data.backtest.stats.cagr)}/><Metric label="Historical MaxDD" value={pct(data.backtest.stats.maxDrawdown)}/><Metric label="Planning proxy" value={pct(data.portfolioConfig.planningProxy.cagr)}/><Metric label="Rolling36 median" value={pct(data.portfolioConfig.planningProxy.rolling36Median)}/><Metric label="Rolling36 worst" value={pct(data.portfolioConfig.planningProxy.rolling36Worst)}/></div><Section title="Stage21 equity"><div style={{height:360}}><ResponsiveContainer width="100%" height="100%"><AreaChart data={chart}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date" minTickGap={40}/><YAxis/><Tooltip/><Area type="monotone" dataKey="equity" stroke="currentColor" fill="currentColor" fillOpacity={0.12}/></AreaChart></ResponsiveContainer></div></Section><p>Planning proxyは同一sample内のstress/rolling検証から作った運用計画用proxyで、True Forward予測値ではありません。</p></div>}
