...
function Overview({ data }: { data: DashboardPayload }) {
  ...
  return <div className="dynamic-stack">
    <Section title="Next Action" action={<StateBadge state={state.state} />} onActivate={() => setShowActionGuide(true)}><div className="next-action"><div><span>次回注文</span><strong>{state.nextAction.type.replaceAll("_", " ")}</strong><p>{state.nextAction.reason}</p></div><div><span>注文予定日</span><strong>{date(state.nextAction.executionDate)}</strong></div><div><span>対象</span><strong>{state.nextAction.symbols.join(" / ") || "—"}</strong><p>{state.nextAction.targetWeights.map((weight) => pct(weight, 0)).join(" / ")}</p></div><div><span>N-PORT取込期限</span><strong>{data.nportOperations?.nextImportDeadlineAt ? updatedAt(data.nportOperations.nextImportDeadlineAt).slice(0, 16) : "—"}</strong><p>{data.nportOperations?.universeMode === "FALLBACK" ? "旧Universeで継続" : "月初選出 16:00 JST"}</p></div></div></Section>
