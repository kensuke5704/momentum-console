...
function MonthlyReturnDistribution() {
  ...
  return <Section title="月別リターン分布" action={<span className="section-asof">N-PORT delay Monte Carlo · {number.format(nportDelayMonteCarlo.paths)} paths</span>}><div className="monthly-return-distribution-scroll"><div className="monthly-return-distribution"><ResponsiveContainer width="100%" height="100%"><BarChart data={monthlyReturns.histogram5Pct} margin={{ top: 10, right: 18, bottom: 8, left: 4 }}><CartesianGrid stroke="#dce1da" vertical={false}/><XAxis dataKey="label" interval={0} angle={-38} textAnchor="end" height={78} tick={{ fontSize: 11 }}/><YAxis width={54} tickFormatter={(value) => `${(Number(value) * 100).toFixed(0)}%`}/><Tooltip cursor={{ fill: "#f0f2ee" }} formatter={(value) => [`${(Number(value) * 100).toFixed(2)}%`, "発生確率"]} labelFormatter={(label) => `月次リターン ${label}`}/><Bar dataKey="probability" name="発生確率" isAnimationActive animationDuration={1_000}>{monthlyReturns.histogram5Pct.map((bin) => <Cell key={bin.label} fill={bin.to <= 0 ? "#c43b31" : bin.from >= 0.049 ? "#167a4b" : "#73776f"}/>)}</Bar></BarChart></ResponsiveContainer></div></div><div className="monthly-return-summary"><span>マイナス月 <strong>{pct(monthlyReturns.negativeProbability, 2)}</strong></span><span>0%月 <strong>{pct(monthlyReturns.zeroProbability, 2)}</strong></span><span>プラス月 <strong>{pct(monthlyReturns.positiveProbability, 2)}</strong></span></div></Section>;
}
