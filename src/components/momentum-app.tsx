...
function Overview({ data }: { data: DashboardPayload }) {
  ...
  return <div className="dynamic-stack">
    <Section title="Next Action" action={<StateBadge state={state.state} />} onActivate={() => setShowActionGuide(true)}><div className="next-action"><div role="button" tabIndex={0} aria-label="Action gateの詳細を開く" onClick={(event) => { event.stopPropagation(); setShowGateGuide(true); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); event.stopPropagation(); setShowGateGuide(true); } }} style={{ cursor: "pointer" }}><span>Action gate</span><div className="next-action-copy"><strong>{oosGate.level}</strong></div></div><div><span>次回注文</span><div className="next-action-copy"><strong>{state.nextAction.type.replaceAll("_", " ")}</strong><p>{state.nextAction.reason}</p></div></div><div><span>対象</span><div className="next-action-copy"><strong>{state.nextAction.symbols.join(" / ") || "—"}</strong><p>{state.nextAction.targetWeights.map((weight) => pct(weight, 0)).join(" / ")}</p></div></div><div><span>注文予定時刻</span><div className="next-action-copy"><strong>{usMarketOpenAt(state.nextAction.executionDate)}</strong></div></div><div><span>N-PORT取込期限</span><div className="next-action-copy"><strong>{data.nportOperations?.nextImportDeadlineAt ? updatedAt(data.nportOperations.nextImportDeadlineAt).slice(0, 16) : "—"}</strong>{data.nportOperations?.universeMode === "FALLBACK" && <p>旧Universeで継続</p>}</div></div></div></Section>
    ...
    {showGateGuide && <div className="modal-backdrop" onMouseDown={() => setShowGateGuide(false)}><div className="action-modal" role="dialog" aria-modal="true" aria-labelledby="gate-guide-title" onMouseDown={(event) => event.stopPropagation()}><header><div><span>ACTION GATE</span><h2 id="gate-guide-title">判断基準</h2></div><button type="button" aria-label="閉じる" onClick={() => setShowGateGuide(false)}><XIcon size={20} /></button></header><div className="action-guide-list"><div className={oosGate.level === "GREEN" ? "current" : ""}><strong>GREEN</strong><p>通常のNext Actionに従って運用を継続します。</p>{oosGate.level === "GREEN" && <span>現在</span>}</div><div className={oosGate.level === "AMBER" ? "current" : ""}><strong>AMBER</strong><p>ルールは変更せず運用を継続し、正式レビューに移行します。</p>{oosGate.level === "AMBER" && <span>現在</span>}</div><div className={oosGate.level === "RED" ? "current" : ""}><strong>RED</strong><p>新規買付を停止し、保有中なら次の米国寄付きで全売却してCashへ移行します。</p>{oosGate.level === "RED" && <span>現在</span>}</div><div><strong>現在の判定根拠</strong><p>{oosGate.reason}</p></div><div><strong>現在のOOS</strong><p>{`観測 ${oosGate.monthsObserved}か月 / MaxDD ${data.oos.asOf ? pct(data.oos.stats.maxDrawdown) : "—"} / CAGR（税引前） ${data.oos.asOf ? pct(data.oos.stats.cagr) : "—"}`}</p></div></div></div></div>}
