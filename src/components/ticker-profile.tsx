...
async function loadProfiles(): Promise<Record<string, CompanyProfile>> {
  ...
  if (!profilePromise) {
    ...
    profilePromise = fetch(`${base}/data/company-profiles.json`, { cache: "no-store" })
      .then(async (response) => response.ok ? ((await response.json()) as CompanyProfileFile).profiles ?? {} : {})
      .catch(() => ({}));
  }
    ...
    <div className={styles.modal} role="dialog" aria-modal="true" aria-labelledby={`ticker-profile-${symbol}`} onMouseDown={(event) => event.stopPropagation()}>
      <header className={styles.header}>
        <div className={styles.headerText}>
          ...
          {!loading && (profile?.industry || profile?.sector) && <p>{profile.industry ?? profile.sector}</p>}
        </div>
      ...
      </div> : <div className={styles.unavailable}>この銘柄の会社情報はまだ取得できていません。</div>}
    </div>
  </div>;
}
  ...
  useEffect(() => {
    const enhance = () => {
      document.querySelectorAll<HTMLElement>(targetSelector).forEach((element) => {
        ...
        element.setAttribute("aria-label", `${text}の会社情報を表示`); element.classList.add(styles.tickerButton);
      });
    };
    const openFromTarget = async (target: EventTarget | null) => {
      ...
      setSymbol(nextSymbol); setProfile(null); setLoading(true);
      const profiles = await loadProfiles(); setProfile(profiles[nextSymbol] ?? null); setLoading(false); return true;
    };
    ...
    const keydown = (event: KeyboardEvent) => {
      ...
      if (!target) return; event.preventDefault(); void openFromTarget(target);
    };
    enhance(); const observer = new MutationObserver(enhance); observer.observe(document.body, { childList: true, subtree: true });
    document.addEventListener("click", click); document.addEventListener("keydown", keydown);
