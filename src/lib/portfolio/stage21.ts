...
function simulateNextOpen(histories:Record<string,PricePoint[]>,snaps:FixedSnap[],regimes:RegimeRow[]):BacktestResult{
 ...
 return{strategyId:PRODUCTION_PORTFOLIO.strategyId,equityCurve:curve,stats:performanceStats(curve),benchmark:null,events};
}
...
export function buildStage21Portfolio(histories:Record<string,PricePoint[]>,universeHistory:UniverseMonth[],cftcRows:CftcPositionRow[]):{backtest:BacktestResult;portfolioState:PortfolioLiveState;innerState:LiveStrategyState}{
 ...
 const g=runBreakoutShadow(histories,universeHistory,START,snaps.at(-1)!.date),regimes=regimeRows(fixedCurve,g,histories.QQQ??[],cftcRows),backtest=simulateNextOpen(histories,snaps,regimes);
