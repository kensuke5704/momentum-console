...
import type { BacktestResult, DashboardPayload, ForwardOosResult, NportOperations, PricePoint, UniverseMonth } from "./types";

const mean = (values: number[]) => values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null;

export function buildDashboardPayload(histories: Record<string, PricePoint[]>, universeHistory: UniverseMonth[], source: "live" | "snapshot" = "live", persisted?: { oos?: ForwardOosResult; frozenBacktest?: BacktestResult; nportOperations?: NportOperations }): DashboardPayload {
