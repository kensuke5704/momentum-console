...
INV=ROOT/'data/research/sec-complete-portfolio-inventory-through-h2-2008.json'
PREF=ROOT/'data/research/sec-etf-registrant-operational-prefilter-through-h2-2008.json'
MANDATORY='2006-02-06'; ASOF='2008-12-31'
...
base=load('catalog_base',ROOT/'scripts/research-sec-id-era-strict-series-source-base-through-h2-2008.py')
...
def prospectus_set(rows):
    ...
    # Frozen semantics: retain latest public supplement at each calendar month-end.
    for year,months in ((2005,range(1,13)),(2006,range(1,13)),(2007,range(1,13)),(2008,range(1,13))):
        for month in months:
            import calendar
...
def main():
    ...
    out={'purpose':'Production-independent post-2006-02-06 Series-ID ETF source shard through the last H2 2008 signal date with the frozen H2-2007 evidence separation and structural binding semantics. Series ID/Class metadata comes only from complete-portfolio filing indexes. ETF operational evidence is same-CIK issuer-own Creation Unit plus exchange listing/trading evidence. Candidate carry-forward is recall-only. No trust-global local-title inheritance, later identity backfill, holdings outcomes, ranks, returns, or strategy results are used.','shard':args.shard,'shards':args.shards,'assignedRegistrantCount':len(assigned),'assignedCiks':assigned,'positiveSeriesCount':len(positives),'positiveSeries':sorted(positives.values(),key=lambda x:x['seriesId']),'sourceOccurrenceCount':len(occurrences),'sourceOccurrences':occurrences,'operationalEvidenceRegistrantCount':len(ops),'prospectusErrorCount':sum('error' in x for x in pros_audit),'sourceErrorCount':sum('error' in x for x in source_audit),'sourceNoScheduleCount':sum('error' not in x and not x.get('hasCompletePortfolioSchedule',False) for x in source_audit),'prospectusAudit':pros_audit,'sourceAudit':source_audit,'masterTransports':trs}
    path=ROOT/'data/research'/f'sec-id-era-strict-series-source-h2-2008-shard-{args.shard}.json';path.parent.mkdir(parents=True,exist_ok=True);path.write_text(json.dumps(out,indent=2)+'\n');print('SUMMARY',json.dumps({k:v for k,v in out.items() if k not in ('assignedCiks','positiveSeries','sourceOccurrences','prospectusAudit','sourceAudit','masterTransports')}),flush=True)
