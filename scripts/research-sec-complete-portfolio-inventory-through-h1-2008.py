...
OUT = ROOT / 'data' / 'research' / 'sec-complete-portfolio-inventory-through-h1-2008.json'
UA = {
...
def parse_master(text: str, quarter: int) -> tuple[list[dict], Counter]:
    ...
    for line in text.splitlines():
        ...
        if form not in ACCEPTED_FORMS or date_filed > CUTOFF:
            continue
...
def main() -> None:
    ...
    for year, quarter in QUARTERS:
        text, transport = load_master(year, quarter)
        rows, forms = parse_master(text, quarter)
        all_rows.extend(rows)
        observed.update(forms)
        transports[f'{year}Q{quarter}'] = transport
        print('QUARTER', json.dumps({
            'year': year, 'quarter': quarter, 'acceptedRows': len(rows),
            'candidateForms': dict(sorted(forms.items())), 'transport': transport,
        }), flush=True)
    all_rows.sort(key=lambda r: (r['dateFiled'], r['form'], r['cik'], r['filename']))
    accessions = {r['accession'] for r in all_rows if r['accession']}
    ciks = {r['cik'] for r in all_rows}
    form_counts = Counter(r['form'] for r in all_rows)
    out = {
        'purpose': (
            'Production-independent cumulative complete-portfolio filing inventory through the last H1 2008 signal date '
            'from official SEC quarterly master indexes. Includes N-Q/N-Q-A and certified annual/semiannual shareholder-report '
            'forms N-CSR/N-CSRS plus amendments from 2006 Q1 through 2008 Q2 filed by 2008-06-30. No known ETF source '
            'accessions, holdings outcomes, ranks, returns, or strategy results are used.'
        ),
        'years': [2006, 2007, 2008],
        'quarters': [f'{y}Q{q}' for y, q in QUARTERS],
        'cutoff': CUTOFF,
        'acceptedForms': sorted(ACCEPTED_FORMS),
        'observedCandidateForms': dict(sorted(observed.items())),
        'transport': transports,
        'filingCount': len(all_rows),
        'uniqueAccessionCount': len(accessions),
        'duplicateAccessionCount': len(all_rows) - len(accessions),
        'uniqueRegistrantCikCount': len(ciks),
