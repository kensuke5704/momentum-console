...
def main():
    ...
    for k,m in sorted(identity_meta.items()):
        ...
        row={'ticker':k[0],'securityId':k[1],'issuer':m.get('issuer'),'asOfReportDate':m.get('asOfReportDate'),'aggregateWeight':float(m.get('aggregateWeight') or 0),'occurrenceCount':int(m.get('occurrenceCount') or 0),'classification':c,'evidence':ev}
        rows.append(row)
        if c=='CONFLICT':conflicts.append(row)
    out={'year':2006,'purpose':'Monotonic cache of positive deterministic PIT issuer-country evidence. UNKNOWN/transport failure never overwrites prior positive evidence; conflicting positive evidence is surfaced as CONFLICT. No strategy return/rank information used.','identityCount':len(rows),'classificationCounts':dict(Counter(r['classification'] for r in rows)),'classificationWeights':{c:sum(r['aggregateWeight'] for r in rows if r['classification']==c) for c in ['US','NON_US','UNKNOWN','CONFLICT']},'conflictCount':len(conflicts),'rows':rows,'conflicts':conflicts}
