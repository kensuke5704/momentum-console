...
# Positive-only textual evidence for issuer types that N-PORT would classify separately from corporate.
# Do not classify generic "Trust" as a fund because REITs/business trusts can be operating equity issuers.
...
def main():
 d=json.loads(INP.read_text());records=d.get('records',d.get('series',[]));rows=[];totalc=0;totalw=0.0;cand=[]
 for r in records:
  for h in r.get('holdings',[]):
   desc=h.get('description') or h.get('issuerName') or '';w=float(h.get('weight') or 0);totalc+=1;totalw+=w
   cats=[]
 ...
 out={'purpose':'Materiality diagnostic for N-PORT ISSUER_TYPE=CORP bridge on 2006 legacy N-Q holdings already restricted to explicit COMMON_EQUITY sections. Identify only positive textual evidence for issuer categories N-PORT separates from corporate. Generic Trust/LP/legal-form strings are not classified. No returns or Universe ranks used.','holdingCount':totalc,'holdingWeight':totalw,'positiveNonCorpNameCount':len(cand),'positiveNonCorpNameCountRate':len(cand)/totalc if totalc else None,'positiveNonCorpNameWeight':sum(x['weight'] for x in cand),'positiveNonCorpNameWeightRate':sum(x['weight'] for x in cand)/totalw if totalw else None,'candidates':cand}
