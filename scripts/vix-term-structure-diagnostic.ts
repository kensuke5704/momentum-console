...
function latestAtOrBefore(xs:PricePoint[],d:string){let lo=0,hi=xs.length-1,best:PricePoint|null=null;while(lo<=hi){const m=(lo+hi)>>1;if(xs[m].date<=d){best=xs[m];lo=m+1;}else hi=m-1;}return best;}
...
async function main(){
 ...
 const vix=(await yh('^VIX')).sort((a,b)=>a.date.localeCompare(b.date)),vix3m=(await yh('^VIX3M')).sort((a,b)=>a.date.localeCompare(b.date)),q=[...market.histories.QQQ].sort((a,b)=>a.date.localeCompare(b.date));
 const obs:any[]=[];
 for(const um of u){
  if(um.asOf<'2020-01-01'||um.asOf>'2026-08-25')continue;
  const sig=buildMonthlySignal({universe:um,histories:market.histories,qqq:q,nextSessionDate:nextUsTradingSession(um.asOf),config:cfg});if(!sig.marketRiskOn||sig.selectedSymbols.length!==2)continue;
  const a=latestAtOrBefore(vix,um.asOf),b=latestAtOrBefore(vix3m,um.asOf);if(!a||!b||b.close<=0)continue;
  // Require both option-index observations to be from the same most-recent US trading date and no later than the signal date.
  if(a.date!==b.date)continue;
  const r=port21(market.histories,sig.selectedSymbols,[.6,.4],um.asOf);if(r==null)continue;
  obs.push({date:um.asOf,vixDate:a.date,ratio:a.close/b.close,backwardation:a.close>b.close,return21:r,symbols:sig.selectedSymbols});
 }
