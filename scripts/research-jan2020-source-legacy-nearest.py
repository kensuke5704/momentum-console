...
OUT=ROOT/'data/research/jan2020-source-legacy-nearest.json'
...
def browse(cik,form):
 ...
 return list(dict.fromkeys(ACC_RE.findall(text)))[:60]
...
def main():
 ...
 out={'purpose':'Find nearest pre-N-PORT legacy report for the exact three ETF series that constitute the 2020-01 Production source set. Registrant CIKs fixed from SEC same-series identity evidence; selection uses only series metadata and dates, not holdings overlap or returns.','targetSeries':len(alltargets),'resolvedSeries':len(chosen),'unresolvedSeries':sorted(alltargets-set(chosen)),'medianDaysToNport':sorted(gaps)[len(gaps)//2] if gaps else None,'chosen':chosen,'candidates':candidates,'audits':audits}
