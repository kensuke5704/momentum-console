...
('AIRR','RBA American Industrial Renaissance','https://www.sec.gov/Archives/edgar/data/1552740/000144554619005047/etf6_ncsr.htm'),
...
def main():
 ...
 for label,title,url in SAMPLES:
  ...
  ls=text.splitlines();keys=[title,label]
  hits=sorted(set(i for i,x in enumerate(ls) if any(k.lower() in x.lower() for k in keys)))
  scored=[]
  for i in hits:
   w='\n'.join(ls[i:min(len(ls),i+1000)])
   score=0
   if re.search(r'Portfolio of Investments|Schedule of Investments',w,re.I):score+=2
   if re.search(r'COMMON STOCKS?',w,re.I):score+=3
   if re.search(r'Net Assets',w,re.I):score+=1
   scored.append((score,i))
  scored.sort(reverse=True);chosen=scored[0][1] if scored else None
  windows=[]
  if chosen is not None:
   windows=[{'line':j,'text':ls[j][:700]} for j in range(max(0,chosen-30),min(len(ls),chosen+700))]
  rec={'label':label,'title':title,'url':url,'transport':tr,'chars':len(text),'lines':len(ls),'titleHits':hits[:30],'scoredHits':scored[:10],'chosenHit':chosen,'windows':windows}
  rows.append(rec);print('DOC',label,'chars',len(text),'chosen',chosen,'scored',scored[:5],flush=True)
  for x in windows[:500]:print(label,x['line'],repr(x['text']),flush=True)
