...
type Path={dates:string[];prices:Record<string,Record<string,PricePoint>>;signals:Record<string,MonthlySignal>}; type Out={cagr:number;dd:number};
const PATHS=Number(process.env.ABLATION_PATHS??100),YEARS=5,H=YEARS*252,WARM=252,BLOCK=20,SEED=20260825,cfg=PRODUCTION_STRATEGY;
...
function stats(curve:number[]):Out{let peak=curve[0]??1,dd=0;for(const v of curve){peak=Math.max(peak,v);dd=Math.min(dd,v/peak-1)}return{cagr:(curve.at(-1)!/(curve[0]||1))**(1/YEARS)-1,dd}}
type Pos={s:string;sh:number;entry:number;stop:number}; type Order={type:"rebalance"|"exit";symbols:string[];weights:number[]}|null;
function simple(path:Path,opt:{fixed:boolean;gate:boolean;stop:boolean;circuit:boolean}):Out{
 let cash=1,pos:Pos[]=[],peak=1,fixed:string[]|null=null,pending:Order=null;const curve:number[]=[];
 const val=(d:string,field:"open"|"close")=>cash+pos.reduce((z,p)=>z+p.sh*(path.prices[d]?.[p.s]?.[field]??path.prices[d]?.[p.s]?.close??p.entry),0);
 const sellOpen=(d:string)=>{if(!pos.length)return;const pp=path.prices[d]??{};cash=pos.reduce((z,p)=>z+p.sh*(pp[p.s]?.open??pp[p.s]?.close??p.entry),0)*(1-cfg.execution.transactionCost);pos=[]};
 const buyOpen=(d:string,syms:string[],ws:number[])=>{const pp=path.prices[d]??{};if(syms.length!==2)return;const px=syms.map(s=>pp[s]?.open);if(px.some(x=>!x||x<=0))return;const base=cash,newPos:Pos[]=[];for(let k=0;k<2;k++){const alloc=base*ws[k]*(1-cfg.execution.transactionCost),p=px[k]!;newPos.push({s:syms[k],sh:alloc/p,entry:p,stop:p*(1-cfg.risk.individualStop)})}pos=newPos;cash=0};
 for(let i=WARM;i<path.dates.length;i++){const d=path.dates[i],pp=path.prices[d]??{};
  if(pending){if(pending.type==="exit")sellOpen(d);else{sellOpen(d);buyOpen(d,pending.symbols,pending.weights);if(opt.circuit)peak=val(d,"open")}pending=null}
  const eq=val(d,"close");peak=Math.max(peak,eq);if(pos.length&&!pending){if(opt.stop&&pos.some(p=>(pp[p.s]?.close??Infinity)<=p.stop))pending={type:"exit",symbols:[],weights:[]};else if(opt.circuit&&eq/peak-1<=-cfg.risk.portfolioCircuit)pending={type:"exit",symbols:[],weights:[]}}
  const sig=path.signals[d];if(sig){let syms=[...sig.selectedSymbols],ws=[...sig.targetWeights];if(opt.fixed){if(!fixed&&syms.length===2)fixed=[...syms];syms=fixed??[];ws=[.5,.5]}const can=syms.length===2&&(!opt.gate||sig.marketRiskOn);pending=can?{type:"rebalance",symbols:syms,weights:ws}:{type:"exit",symbols:[],weights:[]}}
  curve.push(eq)
 }
 return stats(curve)
}
function full(path:Path):Out{let st=initialEngineState(cfg);const qh:PricePoint[]=[],curve:number[]=[];for(let i=0;i<path.dates.length;i++){const d=path.dates[i],next=path.dates[i+1]??null,pp=path.prices[d]??{};if(pp.QQQ)qh.push(pp.QQQ);if(i===WARM)st=initialEngineState(cfg);if(i>=WARM){st=transitionDay(st,{date:d,prices:pp,qqqHistoryThroughClose:qh,monthlySignal:path.signals[d]??null,nextSessionDate:next},cfg);curve.push(st.currentEquity)}}return stats(curve)}
async function main(){const market=JSON.parse(await readFile(resolve("public/data/market-data.json"),"utf8")) as Market,uf=JSON.parse(await readFile(resolve("data/universe-history.json"),"utf8")) as UF,us=[...uf.history].sort((a,b)=>a.asOf.localeCompare(b.asOf)),hist=market.histories,dates=(hist.QQQ??[]).filter(p=>p.date>=us[0].asOf).map(p=>p.date),target=dates.slice(-(WARM+H)),symbols=[...new Set(["QQQ",...us.flatMap(u=>u.symbols.map(x=>x.symbol))])],maps=new Map(symbols.map(s=>[s,new Map((hist[s]??[]).map(p=>[p.date,p]))])),starts=dates.map((_,i)=>i).filter(i=>i>=1&&i+BLOCK<dates.length),r=rng32(SEED);
 ...
 for(let p=0;p<PATHS;p++){const idx:number[]=[];while(idx.length<target.length){const s=starts[Math.floor(r()*starts.length)];for(let j=0;j<BLOCK&&idx.length<target.length;j++)idx.push(s+j)}const syn=new Map(symbols.map(s=>[s,100])),monthly=new Map<string,number[]>(symbols.map(s=>[s,[]])),prices:Record<string,Record<string,PricePoint>>={},signals:Record<string,MonthlySignal>={};
  for(let ti=0;ti<target.length;ti++){const td=target[ti],next=target[ti+1]??null,si=idx[ti],src=dates[si],prev=dates[si-1],day:Record<string,PricePoint>={};for(const s of symbols){const a=maps.get(s)?.get(src),b=maps.get(s)?.get(prev);if(!a||!b||a.open<=0||b.close<=0)continue;const pc=syn.get(s)!,o=pc*(a.open/b.close),c=o*(a.close/a.open);syn.set(s,c);day[s]={date:td,open:o,high:Math.max(o,c),low:Math.min(o,c),close:c} as PricePoint}prices[td]=day;const me=!next||next.slice(0,7)!==td.slice(0,7);if(me){for(const s of symbols)if(day[s])monthly.get(s)!.push(syn.get(s)!);signals[td]=makeSignal(td,next,latestU(us,src),monthly)}}
  const path:Path={dates:target,prices,signals};res[names[0]].push(simple(path,{fixed:true,gate:false,stop:false,circuit:false}));res[names[1]].push(simple(path,{fixed:false,gate:false,stop:false,circuit:false}));res[names[2]].push(simple(path,{fixed:false,gate:true,stop:false,circuit:false}));res[names[3]].push(simple(path,{fixed:false,gate:true,stop:true,circuit:false}));res[names[4]].push(simple(path,{fixed:false,gate:true,stop:true,circuit:true}));res[names[5]].push(full(path));if((p+1)%25===0)console.log(`completed ${p+1}/${PATHS}`)}
 const summary=Object.fromEntries(names.map(n=>{const o=res[n],c=o.map(x=>x.cagr),d=o.map(x=>x.dd);return[n,{cagr:{p05:pct(c,.05),median:pct(c,.5),p95:pct(c,.95)},maxDrawdown:{adverseP05:pct(d,.05),median:pct(d,.5)},probabilities:{cagrGe50:c.filter(x=>x>=.5).length/PATHS,cagrLt0:c.filter(x=>x<0).length/PATHS,ddLe40:d.filter(x=>x<=-.4).length/PATHS}}]}));const out={generatedAt:new Date().toISOString(),paths:PATHS,execution:"signal close -> next session open",summary};await mkdir(resolve("data/research/structural-monte-carlo"),{recursive:true});await writeFile(resolve("data/research/structural-monte-carlo/ablation-v3.json"),JSON.stringify(out,null,2));console.log("RESULT_JSON="+JSON.stringify(out))}
