...
def main():
    h2=json.loads(H2.read_text());h1=json.loads(H1.read_text());pref=json.loads(PREF.read_text());ciks=set(pref['positiveCiks'])
    rows=[r for r in h2['rows'] if r['cik'] in ciks]
    rows += [r for r in h1['rows'] if r['cik'] in ciks and r['dateFiled']<=SOURCE_CUTOFF]
    uniq={}
            ...
            for w in windows:
                ...
                for candidate in strict.title_candidates(w):
                    ...
                    for op in ops.get(r['cik'],[]):
                        binding,distance=strict.classify_binding(candidate['title'],candidate['normalizedTitle'],r['company'],op['lines'])
    ...
    for r in sorted(occ,key=lambda x:(x['evidenceDateFiled'],x['legacyIdentity'],x['evidenceFilename'])):
