...
STATE_RES=[re.compile(r'STATE\s+OF\s+INCORPORATION:\s*([A-Z0-9]{2,3})',re.I),re.compile(r'State\s+of\s+Incorp\.?:\s*([A-Z0-9]{2,3})',re.I)]
...
def header_url(archive_url):
 ...
 return f'https://www.sec.gov/Archives/edgar/data/{cik}/{ad}/{acc}-index-headers.html' if acc else None
def hist_name_ok(target,text):
 names=[m.group(1).strip() for m in NAME_RE.finditer(text)];nt=old.normalize_name(target)
...
def main():
 ...
 out={'purpose':'Fast top-10 UNKNOWN PIT country pilot using current ticker only as exact-name CIK seed; classification requires historical SEC index-headers COMPANY CONFORMED NAME match and filing-time STATE OF INCORPORATION. No current state, returns, or ranks used.','sampleCount':len(results),'resolvedCount':len(resolved),'resolvedWeight':sum(float(r.get('aggregateWeight') or 0) for r in resolved),'sampleWeight':sum(float(r.get('aggregateWeight') or 0) for r in results),'results':results}
