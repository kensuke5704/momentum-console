...
function parse(raw:string):TickerConfig { const i=raw.indexOf(":"); if(i<=0) throw new Error(raw); return {symbol:raw.slice(0,i).toUpperCase(), genre:raw.slice(i+1)}; }
function same(a:string[],b:string[]){ return [...a].sort().join("|") === [...b].sort().join("|"); }

async function main(){
  const candidates=process.argv.slice(2).map(parse);
  ...
  console.log("XWNG_SANITY_JSON_START"); console.log(JSON.stringify(out,null,2)); console.log("XWNG_SANITY_JSON_END");
}
