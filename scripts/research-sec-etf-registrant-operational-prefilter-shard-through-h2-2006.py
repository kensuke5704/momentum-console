...
SRC=ROOT/'data/research/sec-marketwide-nq-inventory-through-h2-2006.json'
SHARD_INDEX=int(os.environ.get('SHARD_INDEX','0'));SHARD_COUNT=int(os.environ.get('SHARD_COUNT','1'))
OUT=ROOT/f'data/research/sec-etf-registrant-operational-prefilter-through-h2-2006-shard-{SHARD_INDEX:02d}.json'
CUTOFF='2006-12-29'
...
def load(ciks):
 ...
 for y,qs in ((2005,range(1,5)),(2006,range(1,5))):
  for q in qs:
...
def main():
 ...
 out={'purpose':'Candidate-only registrant prefilter for every CIK in the fixed official cumulative SEC 2006 N-Q/N-Q-A inventory through the last H2 signal date. Positive requires joint Creation Unit and exchange-listing/trading operational evidence in the deterministic newest core prospectus or newest 497 filed by 2006-12-29. This is not final series-level ETF classification; negatives are not final exclusions until recall is audited. No known source list, holdings, ranks, returns, or strategy outcomes are used.','inventoryArtifactId':INVENTORY_ARTIFACT_ID,'cutoff':CUTOFF,'shardIndex':SHARD_INDEX,'shardCount':SHARD_COUNT,'fullRegistrantCount':len(ciks),'selectedRegistrantCount':len(sel),'registrantsWithProspectusCandidates':sum(bool(bp.get(c)) for c in sel),'positiveCandidateRegistrantCount':len(pos),'positiveCiks':[x['cik'] for x in pos],'masterTransports':trs,'results':results}
