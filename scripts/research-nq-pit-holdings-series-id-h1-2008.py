...
SOURCE = DIR / 'sec-id-era-strict-series-source-h1-2008.json'
ADAPTED = DIR / 'sec-id-era-strict-series-source-h1-2008-holdings-adapter.json'
OUT = DIR / 'nq-pit-holdings-series-id-h1-2008.json'
...
def adapt_source_catalog(source: dict, source_artifact_id: int) -> dict:
    ...
    return {
        'purpose': 'Mechanical Series-ID-only schema adapter for the validated H1 2008 strict source catalog. No source is added, removed, renamed, bridged, or inferred.',
...
def main() -> None:
    ...
    if source_sha != SOURCE_SHA:
        raise RuntimeError(f'unexpected source catalog sha: {source_sha}')
    ...
    result['purpose'] = (
        'H1 2008 Series-ID source-catalog-driven raw complete-portfolio holdings extraction. '
    ...
    result['monthlySourceSeriesAppearanceCount'] = sum(len(s['sourceFilings']) for s in source['monthSnapshots'])
    OUT.write_text(json.dumps(result, indent=2) + '\n')

    print('H1_2008_HOLDINGS_SUMMARY', json.dumps({
        'sourceCatalogArtifactId': source_artifact_id,
        ...
        'monthly': [
            {'signalMonth': s['signalMonth'], 'catalog': s['catalogSourceSeriesCount'], 'records': len(s['sourceFilings']), 'parsed': s['parsedSourceSeriesCount'], 'missing': len(s['missingParsedSeries'])}
