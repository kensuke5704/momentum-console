...
OUT = ROOT / 'data/research/sec-marketwide-nq-inventory-through-h2-2007.json'
UA = {
...
def main():
    ...
    for year, quarter in QUARTERS:
        ...
        print('QUARTER', json.dumps({'year': year, 'quarter': quarter, 'masterRows': master_row_count, 'nqRows': len(qrows), 'uniqueCiks': len({r['cik'] for r in qrows})}), flush=True)
    ...
    out = {
        'purpose': (
            'Production-independent cumulative SEC source inventory through the last 2007 signal date. '
            'All official SEC master-index N-Q/N-Q-A filings from 2006 Q1 through 2007 Q4 filed by 2007-12-31 are listed. '
            'Jul-Dec 2007 monthly snapshots are defined mechanically by filingDate <= the fixed signal date. No known '
