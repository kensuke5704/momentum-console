...
OUT=ROOT/'data/research/sec-filing-index-country-canonical-seed-pilot-2006.json'
...
def canon(s:str)->str:
 ...
 s=re.sub(r'[^A-Z0-9]+',' ',s)
 toks=[CORP_EQ.get(t,t) for t in s.split()]
...
def main():
 data=json.loads(SRC.read_text());unknown=sorted([r for r in data['identityRows'] if r.get('classification')=='UNKNOWN'],key=lambda r:float(r.get('aggregateWeight') or 0),reverse=True)[:100]
 rows=[]
 for row in unknown:
  ...
  seeds=[x for x in cur.CM.get(ticker,[]) if canon(x.get('title') or '')==target]
  if len(seeds)==1:
   seed=seeds[0]
  ...
  rows.append(rec);print('PILOT',json.dumps(rec),flush=True);time.sleep(.04)
 resolved=[r for r in rows if r['classification']!='UNKNOWN']
 out={'purpose':'UNKNOWN-only PIT country pilot. Current ticker metadata seeds CIK only when ticker and issuer title match under fixed corporate-designator equivalences (Corporation/Corp, Incorporated/Inc, Company/Co, Limited/Ltd; punctuation and trailing footnote markers ignored). Country classification is taken only from historical SEC filing-index State of Incorp metadata available by the legacy report date. No current state, fuzzy matching, returns, or ranks used.','sampleCount':len(rows),'resolvedCount':len(resolved),'resolvedWeight':sum(float(r.get('aggregateWeight') or 0) for r in resolved),'sampleWeight':sum(float(r.get('aggregateWeight') or 0) for r in rows),'rows':rows}
