...
OUT=ROOT/'data/research/sec-filing-index-country-smoke-2006.json'
...
def main():
 ...
 for row in unknown:
  ...
  rec['exactSeedCount']=len(exact)
  if len(exact)==1:
   seed=exact[0];rec['seedCik']=seed['cik']
   ...
   for doc in b.get('archiveUrls',[])[:8]:
    for iu in index_urls(doc):
     ...
     if not sm:continue
     code=sm.group(1).upper();rec.update({'classification':'US' if code in cur.old.US_CODES else 'NON_US','stateCode':code,'indexUrl':iu,'transport':tr,'resolutionSource':'CURRENT_TICKER_EXACT_NAME_CIK_THEN_PIT_FILING_INDEX'})
     break
 ...
 out={'purpose':'Five-identity smoke test of the same PIT filing-index State of Incorp. resolver used by the 50-identity pilot. Current metadata only seeds exact ticker+issuer CIK; classification is historical filing-index metadata.','sampleCount':len(rows),'resolvedCount':len(resolved),'resolvedWeight':sum(float(r.get('aggregateWeight') or 0) for r in resolved),'sampleWeight':sum(float(r.get('aggregateWeight') or 0) for r in rows),'rows':rows}
