...
# Same three fixed source submissions as the corrected 2006 PIT baseline.
SOURCES = {
...
COUNTRY_RE = re.compile(r"^(" + "|".join(re.escape(x) for x in sorted(COUNTRY_NAMES, key=len, reverse=True)) + r")\s*(?:--|-)?\s*(?:\d+(?:\.\d+)?%)?\s*$", re.I)
SCHEDULE_RE = re.compile(r"SCHEDULE OF INVESTMENTS", re.I)

# Official CINS first-letter country codes. U is United States, C is Canada.
# For Production US parity, all letters except U are NON_US. U is not accepted as US by itself because
# this diagnostic intentionally avoids using identifier semantics as a positive US proof without issuer-country corroboration.
...
def text_lines(raw: str) -> list[str]:
    # r.jina output is Markdown-like text. Preserve line order and collapse formatting characters.
    lines=[]
...
def source_sections(raw: str) -> list[tuple[str,str]]:
    ...
    for line in text_lines(raw):
        ...
        if m:
            ...
            continue
        # Keep explicit country state through industry subheadings; reset only at clear non-equity terminal sections.
...
def locate_country(desc: str, lines: list[tuple[str,str]]) -> str:
    ...
    hits=[state for text,state in lines if q and q in text]
    hits=[x for x in hits if x!="UNKNOWN"]
...
def main() -> None:
    ...
    for d in mapping.get("details",[]):
        key=(d.get("seriesId"),d.get("reportDate"),d.get("description"))
        by_key[key]=d
    ...
    out={
        "year":2006,
        "purpose":"Structural point-in-time US/NON_US attribution using only explicit legacy N-Q country sections and official CINS country-prefix semantics. No returns, rankings, current domicile, or listing venue used.",
        "rules":[
            "Explicit N-Q country heading is direct point-in-time evidence; UNITED STATES -> US, any other recognized country -> NON_US.",
            "If no explicit N-Q country evidence and a uniquely mapped 9-character security ID is CINS (alphabetic first position), any non-U CINS prefix -> NON_US.",
            "Numeric CUSIP is never treated as US because standard CUSIP also covers Canadian issuers; CINS U is not used as positive US proof in this diagnostic.",
            "All remaining holdings stay UNKNOWN."
        ],
        "holdingCount":total,
        "holdingWeight":total_w,
        "classificationCounts":dict(counts),
        "classificationWeights":dict(weights),
        "resolvedCountRate":(counts["US"]+counts["NON_US"])/total if total else None,
        "resolvedWeightRate":(weights["US"]+weights["NON_US"])/total_w if total_w else None,
        "methods":dict(methods),
        "methodWeights":dict(method_weights),
        "results":results,
    }
