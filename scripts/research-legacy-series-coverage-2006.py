...
def collect_series(obj):
 out=[]
 for key in ('series','filings','results','portfolios'):
  v=obj.get(key)
 ...
 # corrected PIT may be keyed by seriesId
...
def main():
 ...
 pit_series=collect_series(pit)
 if not pit_series:
  # fallback: common corrected artifact grammar stores retainedSeries
  for key in ('retainedSeries','eligibleSeries','seriesRows'):
   for r in pit.get(key,[]) if isinstance(pit.get(key),list) else []:
    if isinstance(r,dict) and r.get('seriesId'):pit_series.append(r['seriesId'])
  pit_series=sorted(set(pit_series))
 idx_series=sorted(set(idx.get('seriesIds',[])))
