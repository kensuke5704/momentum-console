...
OUT=ROOT/'data/research/sec-preid-v29fix-removed-context-diagnostic-2006.json'
    ...
    for x in ids:
        ...
        for i in range(len(lines)):
            for width in (1,2,3):
                ...
                if pos<0:continue
                suffix=n[pos+len(title):].strip()
                nxt=i+width
                hits.append({
                  'line':i,'width':width,'phrase':phrase,'normalizedPhrase':n,
                  'suffix':suffix,'acceptedTitlePhrase':base.accepted_title_phrase(n,title),
        ...
        rows.append({
          'cik':x['cik'],'seriesName':x['seriesName'],'normalizedSeriesName':title,
          'evidenceFilename':fn,'evidenceDateFiled':x['evidenceDateFiled'],'evidenceForm':x['evidenceForm'],
          'oldExplicitEtfClassLineDistance':x.get('explicitEtfClassLineDistance'),'hits':hits[:40],
        })
