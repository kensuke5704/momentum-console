...
for(let index=0; index<tradingDates.length; index++){
  const date=tradingDates[index]; if(date<config.backtestStart) continue;
  ...
  for(const p of before.currentPositions){const pp=prices[p.symbol];if(!pp)continue;const prevClose=p.currentPrice??p.entryPrice;add(p.symbol,p.shares*(pp.open-prevClose));}
  if(actionExec==="SELL_ALL_NEXT_OPEN" || actionExec==="MONTH_END_REBALANCE_NEXT_OPEN") for(const p of before.currentPositions){const pp=prices[p.symbol];if(pp)add(p.symbol,0,p.shares*pp.open*config.execution.transactionCost);}
  ...
  if(actionExec==="BUY_NEXT_OPEN" || actionExec==="MONTH_END_REBALANCE_NEXT_OPEN") for(const p of state.currentPositions){const pp=prices[p.symbol];if(!pp)continue;const invested=p.shares*pp.open,grossAllocation=invested/(1-config.execution.transactionCost),cost=grossAllocation-invested;add(p.symbol,0,cost);getAttr(p.symbol).entries++;const sig=before.pendingSignal,c=sig?.candidates.find(x=>x.symbol===p.symbol);entrySignals.push({date,symbol:p.symbol,targetWeight:p.targetWeight,rank:c?.rank??null,oneMonth:c?.oneMonth??null,threeMonth:c?.threeMonth??null,sixMonth:c?.sixMonth??null,score:c?.score??null,scoreSpread:c?.scoreSpread??null});}
  for(const p of state.currentPositions){const pp=prices[p.symbol];if(!pp)continue;add(p.symbol,p.shares*(pp.close-pp.open));getAttr(p.symbol).holdingDays++;}
}
const rows=[...attrs.entries()].map(([symbol,a])=>({symbol,...a})).sort((a,b)=>b.contribution-a.contribution);
const finalEquity=state.currentEquity,totalNetProfit=finalEquity-1,attributed=rows.reduce((s,x)=>s+x.contribution,0);
for(const row of rows)(row as any).shareOfNetProfit=totalNetProfit!==0?row.contribution/totalNetProfit:null;
const positive=rows.filter(x=>x.contribution>0),positiveTotal=positive.reduce((s,x)=>s+x.contribution,0);
