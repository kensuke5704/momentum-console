...
# Evaluation targets are used only after the registry is constructed.
...
def main():
 text,tr=get()
 # r.jina may prepend markdown; find the CSV header containing Series ID and CIK.
 ...
 for r in rows:
  ...
  # Independent ETF-candidate registry rule: open-end series/class with an exchange ticker and explicit ETF/exchange-traded naming evidence.
  naming=' '.join((sname,cname))
  if org and org!='30':continue
  if not ticker:continue
  if not re.search(r'(^|\W)ETF($|\W)|EXCHANGE[ -]TRADED',naming,re.I):continue
  rec=registry.setdefault(sid,{'seriesId':sid,'cik':cik,'seriesName':sname,'classes':[]})
  rec['classes'].append({'className':cname,'ticker':ticker})
 vals=sorted(registry.values(),key=lambda x:x['seriesId'])
 found={x['seriesId']:x for x in vals if x['seriesId'] in TARGETS}
 ...
 print('SUMMARY',json.dumps({k:v for k,v in out.items() if k not in ('foundTargets','registrySample')}),flush=True)
