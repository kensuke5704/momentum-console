...
OUT=DIR/'sec-etf-registrant-operational-prefilter-through-h2-2008.json'
PRIOR=DIR/'sec-etf-registrant-operational-prefilter-through-h1-2008.json'
SOURCE_RUN_ID=34376199155
INVENTORY_ARTIFACT_ID=10112816094
PRIOR_INPUT_ARTIFACT_ID=10085971852


def main():
    files=sorted(DIR.glob('sec-etf-registrant-operational-prefilter-through-h2-2008-shard-*.json'))
    ...
    for cik in positive_ids:
        if cik in current_by_cik:
            positive_candidates.append({**current_by_cik[cik], 'candidateSource':'CURRENT_THROUGH_H2_2008'})
        else:
            if cik not in prior_by_cik:
                raise RuntimeError(f'prior positive candidate payload missing for {cik}')
            positive_candidates.append({**prior_by_cik[cik], 'candidateSource':'H1_2008_CARRY_FORWARD'})

    out={
        'purpose':'Deterministic merge of the 12 cumulative-through-H2-2008 registrant operational-evidence prefilter shards with monotone authoritative H1-2008 candidate carry-forward. The prefilter is recall-oriented only: a registrant with PIT-valid prior candidate evidence remains eligible for downstream review even when a newer filing no longer repeats broad ETF language. Newly discovered H2-2008 candidate CIKs do not alter any closed 2006-01 through 2008-06 source snapshot. Final positive Series still require the unchanged strict issuer-own Series-level evidence and binding rules. No negative registrant is treated as a final ETF exclusion by this artifact.',
