...
if isinstance(boot,list): filings=boot
elif isinstance(boot,dict): filings=boot.get('snapshots',boot.get('filings',boot.get('records',[])))
else: filings=[]
by_acc={f.get('accession'):f for f in filings if isinstance(f,dict)}
rows=[]
for s in jan.get('sourceFilings',[]):
    b=by_acc.get(s.get('accession'),{})
    rows.append({'accession':s.get('accession'),'seriesId':s.get('seriesId'),'seriesName':s.get('seriesName'),'filingDate':s.get('filingDate'),'reportDate':b.get('reportDate'),'holdingCount':len(b.get('holdings',[])) if isinstance(b.get('holdings'),list) else None,'bootstrapSeriesName':b.get('seriesName')})
print('SUMMARY',json.dumps({'signalMonth':'2020-01','asOf':jan.get('asOf'),'universeSize':len(jan.get('symbols',[])),'sourceSeriesCount':len(rows),'uniqueSeries':len({r['seriesId'] for r in rows}),'bootstrapSnapshots':len(filings)}),flush=True)
