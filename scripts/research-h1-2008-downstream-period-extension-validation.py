...
PARSER_INVARIANCE = DATA / 'nq-holdings-parser-invariance-h2-2007-h1-2008.json'
...
PARSER_INVARIANCE_RUN_ID = 34321240803
PARSER_INVARIANCE_ARTIFACT_ID = 10091932913
    ...
    for filing in snapshot.get('sourceFilings', []):
        for holding in filing.get('holdings', []):
            ...
            for evidence in holding.get('countryEvidence') or []:
                ...
                if date_filed: yield date_filed
...
def main() -> None:
    ...
    overlap=parser_invariance.get('sourceCatalogOverlapCount')
    checks['parserInvariancePassed']=(
        parser_invariance.get('passed') is True
        and parser_invariance.get('leftSourceArtifactId')==10041257985
        and parser_invariance.get('rightSourceArtifactId')==SOURCE_ARTIFACT_ID
        and parser_invariance.get('leftHoldingsArtifactId')==10041328876
        and parser_invariance.get('rightHoldingsArtifactId')==HOLDINGS_ARTIFACT_ID
        and isinstance(overlap,int) and overlap>0
        and parser_invariance.get('holdingsOverlapCount')==overlap
    ...
    map_months=mapping.get('monthSnapshots',[]); country_months=country.get('monthSnapshots',[])
    diagnostic_months=diagnostic.get('monthSnapshots',[]); builder_months=builder.get('monthSnapshots',[])
    checks['sixMonthCoverage']=all(len(rows)==6 for rows in (map_months,country_months,diagnostic_months,builder_months))
    cardinality_ok=True; no_lookahead=True; corp_ok=True; exact_parity=True; rank_ok=True; monthly=[]
    if checks['sixMonthCoverage']:
        for expected,mapped,resolved,reference,built in zip(EXPECTED,map_months,country_months,diagnostic_months,builder_months):
            ...
            same_counts=(isinstance(source_count,int) and resolved.get('sourceSeriesCount')==source_count and reference.get('sourceSeriesCount')==source_count and built.get('sourceSeriesCount')==source_count and mapped.get('commonEquityHoldingCount')==resolved.get('commonEquityHoldingCount'))
            cardinality_ok=cardinality_ok and same_period and same_counts
            dates=list(evidence_dates(resolved)); month_no_lookahead=all(d<=as_of for d in dates); no_lookahead=no_lookahead and month_no_lookahead
            month_corp_ok=resolved.get('corpPositiveNonCorpNameCount')==0; corp_ok=corp_ok and month_corp_ok
            month_exact=(reference.get('primaryEligibleSourceSeriesCount')==built.get('eligibleSourceSeriesCount') and reference.get('primarySymbols')==built.get('symbols'))
            exact_parity=exact_parity and month_exact
            symbols=built.get('symbols',[]); month_rank=(len(symbols)<=80 and [r.get('universeRank') for r in symbols]==list(range(1,len(symbols)+1))); rank_ok=rank_ok and month_rank
            monthly.append({'signalMonth':month,'asOf':as_of,'sourceSeriesCount':source_count,'eligibleSourceSeriesCount':built.get('eligibleSourceSeriesCount'),'universeSize':len(symbols),'maxCountryEvidenceDate':max(dates) if dates else None,'corpPositiveNonCorpNameCount':resolved.get('corpPositiveNonCorpNameCount'),'exactBuilderParity':month_exact})
    else:
    ...
    pit=country.get('npxIssuerVariantPitAudit') or {}; blocked=pit.get('blockedFutureIssuerVariantOccurrencesBySignal') or {}
    checks['npxIssuerVariantPitBounded']=(isinstance(pit.get('incrementalRecordCountInJuneContainer'),int) and all(k in {d for _,d in EXPECTED} for k in blocked) and blocked.get('2008-06-30',0)==0)

    passed=all(checks.values())
    report={'purpose':'H1 2008 downstream period-extension exact-parity validation. Verifies parser invariance, fixed source/holdings/mapping/N-PX/country lineage, strict PIT country no-lookahead, CORP bridge invariance, and exact six-month equality between the frozen historical-Universe builder and the independent frozen primary diagnostic. No strategy outcome is read.','definition':'docs/research/h1-2008-downstream-period-extension-validation-definition.md','sourceCatalogSha256':SOURCE_SHA,'parserInvarianceRunId':PARSER_INVARIANCE_RUN_ID,'parserInvarianceArtifactId':PARSER_INVARIANCE_ARTIFACT_ID,'checks':checks,'monthly':monthly,'passed':passed}
