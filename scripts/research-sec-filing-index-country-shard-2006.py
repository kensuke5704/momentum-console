...
SHARD=int(os.environ.get('SHARD_INDEX','0'));COUNT=int(os.environ.get('SHARD_COUNT','12'))
OUT=ROOT/f'data/research/sec-filing-index-country-shard-{SHARD}-2006.json'
...
def main():
 ...
 unknown=sorted([r for r in data['identityRows'] if r.get('classification')=='UNKNOWN'],key=lambda r:(str(r.get('ticker') or ''),str(r.get('securityId') or ''),str(r.get('issuer') or '')))
 subset=[r for i,r in enumerate(unknown) if i%COUNT==SHARD]
 rows=[]
 for row in subset:
  ...
  rows.append(rec);print('ROW',json.dumps(rec),flush=True);time.sleep(.04)
 resolved=[r for r in rows if r['classification']!='UNKNOWN']
 out={'purpose':'Shard of remaining UNKNOWN mapped identities. Current ticker metadata seeds CIK only on unique exact normalized ticker+issuer match; classification requires historical SEC filing-index State of Incorp. by legacy report date. Existing resolved classifications remain frozen. No current state, returns, or ranks used.','shardIndex':SHARD,'shardCount':COUNT,'rowCount':len(rows),'resolvedCount':len(resolved),'resolvedWeight':sum(float(r.get('aggregateWeight') or 0) for r in resolved),'rows':rows}
