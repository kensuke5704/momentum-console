...
H2_HOLDINGS = DATA / 'nq-pit-holdings-series-id-h2-2006.json'
H1_2007_HOLDINGS = DATA / 'nq-pit-holdings-series-id-h1-2007.json'
H2_SOURCE = DATA / 'sec-id-era-strict-series-source-h2-2006.json'
H1_2007_SOURCE = DATA / 'sec-id-era-strict-series-source-h1-2007.json'
OUT = DATA / 'nq-holdings-parser-invariance-h2-2006-h1-2007.json'
...
def parser_semantic_view(row: dict) -> dict:
    # legacyIdentity is schema/catalog bridge metadata, not raw parser output.
    return {key: value for key, value in row.items() if key != 'legacyIdentity'}


def main() -> None:
    h2 = json.loads(H2_HOLDINGS.read_text())
    h1_2007 = json.loads(H1_2007_HOLDINGS.read_text())
    h2_source = json.loads(H2_SOURCE.read_text())
    h1_2007_source = json.loads(H1_2007_SOURCE.read_text())

    left = unique_holdings_records(h2)
    right = unique_holdings_records(h1_2007)
    holdings_overlap = sorted(set(left) & set(right))
    source_overlap = sorted(unique_source_keys(h2_source) & unique_source_keys(h1_2007_source))
    ...
    for key in holdings_overlap:
        ...
        if lrow == rrow:
            ...
            legacy_identity_differences.append({
                ...
                'h2LegacyIdentity': lrow.get('legacyIdentity'),
                'h1_2007LegacyIdentity': rrow.get('legacyIdentity'),
            })
    ...
    report = {
        'purpose': (
            'Parser-invariance audit across validated H2-2006 and H1-2007 Series-ID raw holdings. '
        ...
        'h2UniqueHoldingsFilingRecordCount': len(left),
        'h1_2007UniqueHoldingsFilingRecordCount': len(right),
    ...
    print('PARSER_INVARIANCE_H2_2006_H1_2007', json.dumps({
        key: report[key] for key in (
            'h2UniqueHoldingsFilingRecordCount',
            'h1_2007UniqueHoldingsFilingRecordCount',
