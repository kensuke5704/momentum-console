    ...
    for token_line in op_index['tokenLines'].get(parts[0],()):
        for i in range(max(0,token_line-2),token_line+1):
            for width in (1,2,3):
                ...
                if accepted:exact_hits.add(i)
                pos=phrase.find(normalized_title)
                if pos<0:continue
                suffix=phrase[pos+len(normalized_title):].strip()
                # A multi-line phrase is a direct class association only when the
                # complete title+suffix also satisfies the existing strict title
                # grammar. A nearby ETF/VIPER token can belong to another Series.
                if accepted and suffix and CLASS_TOKEN.search(suffix):direct.append((i,'TITLE_PHRASE_CARRIES_CLASS'))
