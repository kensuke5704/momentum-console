...
SRC=ROOT/'data/research/sec-marketwide-nq-inventory-through-h1-2008.json'
SHARD_INDEX=int(os.environ.get('SHARD_INDEX','0'));SHARD_COUNT=int(os.environ.get('SHARD_COUNT','1'))
OUT=ROOT/f'data/research/sec-etf-registrant-operational-prefilter-through-h1-2008-shard-{SHARD_INDEX:02d}.json'
CUTOFF='2008-06-30'
...
def load(ciks):
 ...
 periods=((2005,range(1,5)),(2006,range(1,5)),(2007,range(1,5)),(2008,range(1,3)))
 for y,qs in periods:
  for q in qs:
...
def main():
 ...
 out={'purpose':'Candidate-only registrant prefilter for every CIK in the fixed official cumulative SEC N-Q/N-Q-A inventory through the last H1 2008 signal date. Positive requires joint Creation Unit and exchange-listing/trading operational evidence in the deterministic newest core prospectus or newest 497 filed by 2008-06-30. This is not final Series-level ETF classification; prior PIT-valid candidates are handled only by the separate recall merge and newly discovered H1-2008 CIKs may not alter the closed 2006-2007 source history. No known source list, holdings, ranks, returns, or strategy outcomes are used.','inventoryArtifactId':INVENTORY_ARTIFACT_ID,'cutoff':CUTOFF,'shardIndex':SHARD_INDEX,'shardCount':SHARD_COUNT,'fullRegistrantCount':len(ciks),'selectedRegistrantCount':len(sel),'registrantsWithProspectusCandidates':sum(bool(bp.get(c)) for c in sel),'positiveCandidateRegistrantCount':len(pos),'positiveCiks':[x['cik'] for x in pos],'masterTransports':trs,'results':results}
