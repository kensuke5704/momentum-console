...
# Read only relevant tickers, P purchases, filing-date range, and officer/director rows through HTTP range scans.
...
where t.form_type='4' and t.transaction_code='P' and t.acquired_disposed='A'
 and (t.is_director is true or t.is_officer is true)
...
for x in u:
 ...
 for m in x['symbols']:
  ...
  # filed_at < signal date: exclude same-date filing because timestamp is unavailable.
...
out={'generatedAt':__import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat(),'validity':{'researchOnly':True,'trueOOS':False,'pit':True,'freeDataOnly':True,'diagnosticOnly':True,'source':'SEC Form 4 bulk data via DataForge mirror','rawOnly':True,'rule':'Form4 code P + acquired A + director/officer + value>0; unique buyers filed in prior 30 calendar days; same-day filings excluded; future starts next open','parameterSearch':False},'coverage':{'dynamicSymbols':len(syms),'rawPurchaseRows':len(rows),'tickersWithPurchases':len(by),'observations':len(obs)},'diagnostic':{'early2020_2023':summ('2020-01-01','2023-12-31'),'late2024_2026':summ('2024-01-01','2026-08-25')},'rows':obs}
