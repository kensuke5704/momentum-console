...
# Role flags in the mirror are not populated for P transactions. A Form 4 filer is already a Section 16 reporting insider,
# so this diagnostic uses all reported open-market P acquisitions rather than an unreliable derived role flag.
...
out={'generatedAt':__import__('datetime').datetime.now(__import__('datetime').timezone.utc).isoformat(),'validity':{'researchOnly':True,'trueOOS':False,'pit':True,'freeDataOnly':True,'diagnosticOnly':True,'source':'SEC Form 4 bulk data via DataForge mirror','rawOnly':True,'rule':'Form4 code P + acquired A + value>0; unique reporting owners filed in prior 30 calendar days; same-day filings excluded; future starts next open','roleFlagIssue':'mirror is_director/is_officer fields are unpopulated for P rows; role filter intentionally removed before observing any valid return diagnostic','parameterSearch':False},'coverage':{'dynamicSymbols':len(syms),'rawPurchaseRows':len(rows),'tickersWithPurchases':len(by),'observations':len(obs)},'diagnostic':{'early2020_2023':summ('2020-01-01','2023-12-31'),'late2024_2026':summ('2024-01-01','2026-08-25')},'rows':obs}
