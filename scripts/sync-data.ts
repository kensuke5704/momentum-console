...
import { fetchNasdaqAssetManagerPositions } from "../src/lib/cftc";
...
async function main() {
  ...
  const symbols = [...new Set(["QQQ", "TQQQ", "GLDM", ...universeHistory.flatMap((month) => month.symbols.map((member) => member.symbol))])];
  const currentSymbols = [...new Set(universeHistory.at(-1)?.symbols.map((member) => member.symbol) ?? [])];
  const intradaySymbols = [...new Set(["QQQ", "TQQQ", "GLDM", ...currentSymbols])];
  ...
  const latestPrices = Object.fromEntries(intradaySymbols.flatMap((symbol) => { const point=fetchedIntraday[symbol]?.at(-1);return point?[[symbol,{price:point.close,asOf:point.timestamp}]]:[]; }));
  ...
  if (candidateDate) {
    ...
    const required = new Set<string>(["QQQ", "GLDM", ...priorState.currentPositions.map((position) => position.symbol)]);
    if (priorState.nextAction.executionDate === candidateDate) for (const symbol of priorState.nextAction.symbols) required.add(symbol);
    if (monthlyUniverse) for (const member of monthlyUniverse.symbols) required.add(member.symbol);
    const missing = [...required].filter((symbol) => !histories[symbol]?.some((point) => point.date === candidateDate));
    if (missing.length) { histories = { ...histories, QQQ: histories.QQQ.filter((point) => point.date !== candidateDate) }; warning = `${candidateDate} daily close was not activated because required Stage21 prices are incomplete: ${missing.join(", ")}`; }
  }

  const latestObservedDate = [historySnapshots.QQQ?.pendingLatest?.date,fetchedIntraday.QQQ?.at(-1)?.timestamp.slice(0,10)].filter((value):value is string=>Boolean(value)).sort().at(-1);
  const activatedDate = histories.QQQ?.at(-1)?.date;
  if (latestObservedDate && (!activatedDate || latestObservedDate > activatedDate)) warning = warning ?? `${latestObservedDate} Yahoo daily close is incomplete and the validated regular-close fallback did not pass.`;
  const requireLatestClose = ["1", "true"].includes(process.env.REQUIRE_LATEST_DAILY_CLOSE ?? "");
  if (requireLatestClose) {
    const parts = Object.fromEntries(new Intl.DateTimeFormat("en-CA", { timeZone:"America/New_York",year:"numeric",month:"2-digit",day:"2-digit" }).formatToParts(new Date()).filter((part)=>part.type!=="literal").map((part)=>[part.type,part.value]));
    const todayEt=`${parts.year}-${parts.month}-${parts.day}`,expectedSession=previousUsTradingSession(todayEt);
    if (!activatedDate || activatedDate < expectedSession) warning = warning ?? `Latest activated daily close is ${activatedDate ?? "none"}; expected at least ${expectedSession} before the next US open.`;
  }
  if (warning) console.warn(`MARKET_DATA_NOT_READY ${warning}`);
  if (warning && requireLatestClose) throw new Error(warning);

  const dashboard = buildDashboardPayload(histories, universeHistory, cftcRows, "live", { oos, frozenBacktest: frozen?.backtest, nportOperations, latestPrices });
  ...
  await writeFile(resolve("public/data/live-state.json"), `${JSON.stringify({innerFixed60:dashboard.liveState,stage21:dashboard.portfolioState})}\n`);
  await writeFile(profilePath, `${JSON.stringify({ generatedAt: dashboard.generatedAt, profiles: companyProfiles })}\n`);
  console.log(`Saved ${outputPath}; Stage21 ${dashboard.portfolioState.regime}; next ${dashboard.portfolioState.nextAction.type}; Fixed60 signal ${dashboard.currentSignal?.signalDate ?? "none"}`);
}
