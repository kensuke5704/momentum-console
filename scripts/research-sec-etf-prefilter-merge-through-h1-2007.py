...
OUT=DIR/'sec-etf-registrant-operational-prefilter-through-h1-2007.json'
PRIOR=DIR/'sec-etf-registrant-operational-prefilter-through-h2-2006.json'
SOURCE_RUN_ID=34183876207
INVENTORY_ARTIFACT_ID=10039805856
PRIOR_INPUT_ARTIFACT_ID=10038188212


def main():
    files=sorted(DIR.glob('sec-etf-registrant-operational-prefilter-through-h1-2007-shard-*.json'))
    ...
    for cik in positive_ids:
        if cik in current_by_cik:
            positive_candidates.append({**current_by_cik[cik], 'candidateSource':'CURRENT_THROUGH_H1_2007'})
        else:
            positive_candidates.append({**prior_by_cik[cik], 'candidateSource':'H2_2006_CARRY_FORWARD'})

    out={
        'purpose':'Deterministic merge of the 12 cumulative-through-H1-2007 registrant operational-evidence prefilter shards with monotone authoritative H2-2006 candidate carry-forward. The prefilter is recall-oriented only: a registrant with PIT-valid prior candidate evidence remains eligible for downstream review even when a newer filing no longer repeats broad ETF language. Final positive Series still require the unchanged strict issuer-own Series-level evidence and binding rules. No negative registrant is treated as a final ETF exclusion by this artifact.',
