...
test("Forward OOS starts on the Fixed60 frozen production date", () => {
  const empty = emptyForwardOos(PRODUCTION_STRATEGY.strategyId);
  assert.equal(empty.startedAt, OOS_START_DATE);
  assert.equal(OOS_START_DATE, "2026-08-31");
...
test("Forward OOS uses actual post-start equity and appends without rewriting confirmed dates", () => {
  const first = updateForwardOos(backtest([
    { date: "2026-08-28", equity: 18, drawdown: 0 },
    { date: "2026-08-31", equity: 20, drawdown: 0 },
    { date: "2026-09-01", equity: 22, drawdown: 0 },
  ]));
  assert.equal(first.baselineBacktestEquity, 20);
  assert.deepEqual(first.equityCurve.map((point) => [point.date, point.equity]), [["2026-08-31", 1], ["2026-09-01", 1.1]]);

  const appended = updateForwardOos(backtest([
    { date: "2026-08-31", equity: 20, drawdown: 0 },
    { date: "2026-09-01", equity: 99, drawdown: 0 },
    { date: "2026-09-02", equity: 24, drawdown: 0 },
  ]), first);
  assert.equal(appended.equityCurve.find((point) => point.date === "2026-09-01")?.equity, 1.1);
  assert.equal(appended.equityCurve.find((point) => point.date === "2026-09-02")?.equity, 1.2);
  assert.equal(appended.asOf, "2026-09-02");
});

test("a prior-strategy OOS series is not carried into Fixed60", () => {
  const stale = { ...emptyForwardOos("momentum-dynamic-2026-08-v1"), startedAt: "2026-08-25" };
  const updated = updateForwardOos(backtest([
    { date: "2026-08-31", equity: 20, drawdown: 0 },
  ]), stale);
  assert.equal(updated.strategyId, PRODUCTION_STRATEGY.strategyId);
  assert.equal(updated.startedAt, OOS_START_DATE);
